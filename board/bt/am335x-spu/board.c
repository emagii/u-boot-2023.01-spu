 ...
 * board.c
 *
 * Board functions for BT AM335X based boards
 *
...
#define GPIO_DDR_VTT_EN		GPIO_TO_PIN(2, 25)
...
#define	MMC1_CD_GPIO		GPIO_TO_PIN(
...
static const struct ddr_data ddr3_spu_data = {
...
static const struct cmd_control ddr3_spu_cmd_ctrl_data = {
...
static struct emif_regs ddr3_spu_emif_reg_data = {
...
const struct ctrl_ioregs ioregs_spu = {
...
void sdram_init(void)
{
	if (board_is_spu()) {
		...
		gpio_direction_output(GPIO_DDR_VTT_EN, 1);	// Enable VTT_EN
		for (u32 i = 0; i < 100 ; i++) {
			udelay(50);				// Wait 5 ms.
		}
	}
	...
	if (board_is_spu())
		config_ddr(400, &ioregs_spu,
			   &ddr3_spu_data,
			   &ddr3_spu_cmd_ctrl_data,
			   &ddr3_spu_emif_reg_data, 0);
	for (u32 wait_sdram = 0; wait_sdram < 100 ; wait_sdram++) {
		udelay(50);					// Wait 5 ms.
	}
}
#endif
...
int board_eth_init(struct bd_info *bis)
{
	...
	printk("eth: Using external refclk for rmii2\n");
...
{
	if (board_is_spu() && !strcmp(name, "am335x-spu"))
		return 0;
	else
		return -1;
}
